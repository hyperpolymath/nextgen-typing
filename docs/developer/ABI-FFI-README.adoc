...
# NEXTGEN_TYPING ABI/FFI Documentation
...
idris2 --cg c-header Types.idr -o ../../../generated/abi/nextgen_typing.h
```
...
Compile with:
```bash
gcc -o example example.c -lnextgen_typing -L./zig-out/lib
```

### From Idris2

```idris
import NEXTGEN_TYPING.ABI.Foreign
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/nextgen_typing.h
   ```
